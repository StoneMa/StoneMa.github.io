...
在神经网络中，**激活函数的作用是能够给神经网络加入一些非线性因素，使得神经网络可以更好地解决较为复杂的问题。**这个Activation Function翻译过来叫做激活函数，一开始的时候我真的以为它是要激活什么东西，比如前面各层神经元或者感知器的输出到达某个值以后才出发什么东西之类的，后来慢慢理解到它并不是为了激活什么，而目的是增加网络结构的非线性能力。
我们看一下常用的激活函数:
$$sigmoid\ funtion: f(z) = \frac{1}{1+e^{-x}}$$
## 什么是异或，线性可分与不可分问题，？
## 卷积神经网络中的分层
## 输入层（Input Layer）
